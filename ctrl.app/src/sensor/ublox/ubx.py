...
class UbxStream(object):
    ...
    @rate.setter
    def rate(self,rate):
        y = UbxMessage('06','08', msg_type="tx", rate=rate, version=self._version)
        try:
            if(self.dev.writable):
                if(self.dev.write(y.msg)):
                    time.sleep(1)  # Sleep to make sure buffer gets written!
    ...
    def enable_message(self, msgClass, msgId):
        msg = UbxMessage('06', '01', msg_type="tx", msgClass=msgClass, msgId=msgId, ioPorts=[1, 1, 1, 1, 1, 1])
...
class UbxMessage(object):
        ...
        if(msg_type == "rx"):
            ...
            #CFG
                ...
                message = {'00': lambda: self.__ubx_CFG_PRT(kwargs["rate"]),
                           '08': lambda: self.__ubx_CFG_RATE(kwargs["rate"]),
    ...
    def __ubx_CFG_PRT(self, rate):
        ...
        payload = [length, 0, uart_port, 0, 0, 0, 208, 8, 0, 0, rate1, rate2, rate3, rate4, 1, 0, 3, 0, 0, 0, 0, 0] # only UBX in
    ...
    # UBX-CFG-MSG (0x06 0x08)
    def __ubx_CFG_RATE(self,  rate):
        ...
        while(len(rate) < 4):
            rate = '0' + rate
        rate1, rate2 = int(rate[-2:], 16), int(rate[-4:-2], 16)

        payload = [length, 0, rate1, rate2, 1, 0, 0, 0]
        checksum = self.__calc_checksum(ubx_class, ubx_id, payload)
        payload = payload + checksum
        try:
